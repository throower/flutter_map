...
class SelectedPolygonLayerOptions extends LayerOptions {
  final List<SelectedPolygon> polygons;
...
class SelectedPolygonLayerWidget extends StatelessWidget {
  final SelectedPolygonLayerOptions options;
  SelectedPolygonLayerWidget({Key? key, required this.options}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final mapState = MapState.maybeOf(context)!;
    return SelectedPolygonLayer(options, mapState, mapState.onMoved);
  }
}

class SelectedPolygonLayer extends StatelessWidget {
  final SelectedPolygonLayerOptions polygonOpts;
  final MapState map;
  final Stream<Null>? stream;

  SelectedPolygonLayer(this.polygonOpts, this.map, this.stream)
    ...
    return StreamBuilder(
      ...
      builder: (BuildContext context, _) {
        ...
        for (var polygon in polygonOpts.polygons) {
          ...
          polygons.add(
            CustomPaint(
              painter: SelectedPolygonPainter(polygon),
              size: size,
            ),
          );
        }
...
class SelectedPolygonPainter extends CustomPainter {
  final SelectedPolygon polygonOpt;

  SelectedPolygonPainter(this.polygonOpt);
  ...
  @override
  bool shouldRepaint(SelectedPolygonPainter other) => false;
